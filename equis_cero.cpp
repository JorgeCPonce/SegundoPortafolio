...
char ganadorColumnas(char yablero[][LADO]);
...
void jugar(){
	...
	while (casillasLibres >0 && ganador ==0){
		if (turnoJugador){casillasLibres -= marcarCasillaJugador(tablero);
		}else {casillasLibres -= marcarCasillaComputadora(tablero);
		}
		turnoJugador=!turnoJugador;
		ganador = determinarGanador(tablero);
	}
	switch (ganador){case 'X':
